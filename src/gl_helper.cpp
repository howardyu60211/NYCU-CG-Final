...
// Helper to load raw data (CPU side)
...
// Helper to create texture from raw data (GPU side)
GLuint createTextureFromData(unsigned char* data, int width, int height, int nrChannels) {
  if (!data) return 0;
...
GLuint createTexture(const char* filename, GLint wrapMode) {
  int width, height, nrChannels;
  unsigned char* data = loadTextureCPU(filename, &width, &height, &nrChannels);
  if (!data) return 0;

  GLuint texture = createTextureFromData(data, width, height, nrChannels);

  // Apply wrap mode
  if (texture != 0 && wrapMode != GL_REPEAT) {
    glBindTexture(GL_TEXTURE_2D, texture);
